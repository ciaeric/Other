...
Lib.Filters.azerite = {
	keyword = "azerite", --C_CurrencyInfo.GetBasicCurrencyInfo(C_CurrencyInfo.GetAzeriteCurrencyID()).name:lower(),

	canSearch = function(self, operator, search)
		return false; --return not operator and self.keyword:find(search)
	end,

	match = function(self, link)
		return false; --return C_AzeriteItem.IsAzeriteItemByID(link) or C_AzeriteEmpoweredItem.IsAzeriteEmpoweredItemByID(link)
	end
}
