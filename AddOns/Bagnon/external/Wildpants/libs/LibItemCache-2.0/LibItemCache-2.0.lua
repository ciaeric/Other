...
function Lib:GetBagInfo(owner, bag)
	...
	elseif  bag == BACKPACK_CONTAINER or bag == BANK_CONTAINER then
		--print(bag, REAGENTBANK_CONTAINER, BACKPACK_CONTAINER, BANK_CONTAINER)
	...
	elseif isguild then
		...
		elseif bag ~= BACKPACK_CONTAINER and bag ~= BANK_CONTAINER and bag < 11 then
			...
			if (item.link) then
				...
				isCraftingReagent = GetItemInfo(item.link ) 

				item.itemClassID = itemClassID
				item.itemSubClassID = itemSubClassID

				--print(item.slot, item.link, item.itemClassID, itemType, item.itemSubClassID, itemSubType  )
			end
		end
	end
