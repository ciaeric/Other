...
function ItemSlot:Create()
	...
	item.BattlepayItemTexture:Hide()
...
function ItemSlot:Construct(id)
	...
	--return CreateFrame('Button', ADDON..self.Name..id, nil, 'ItemButtonTemplate')
end
...
function ItemSlot:UpdateBorder()
	...
	local new = false --Addon.sets.glowNew and self:IsNew()
	...
	self.BattlepayItemTexture:SetDesaturated(false)
	self.BattlepayItemTexture:SetShown(false)
	local Bag = Addon.Bag
	local bag = self:GetBagInfo()
	if(bag) then
		if ( bag.itemClassID == Bag.ClassID["Container"] ) then
		
			if(bag.itemSubClassID == Bag.ContainerSubClassID["Soul Bag"] ) then
				self.BattlepayItemTexture:SetDesaturated(true)
				self.BattlepayItemTexture:SetShown(true)
			elseif(bag.itemSubClassID == Bag.ContainerSubClassID["Herb Bag"] ) then
				self.BattlepayItemTexture:SetDesaturated(true)
				self.BattlepayItemTexture:SetShown(true)

			elseif(bag.itemSubClassID == Bag.ContainerSubClassID["Enchanting Bag"] ) then
				self.BattlepayItemTexture:SetDesaturated(true)
				self.BattlepayItemTexture:SetShown(true)

			elseif(bag.itemSubClassID == Bag.ContainerSubClassID["Engineering Bag"] ) then
				self.BattlepayItemTexture:SetDesaturated(true)
				self.BattlepayItemTexture:SetShown(true)

			elseif(bag.itemSubClassID == Bag.ContainerSubClassID["Mining Bag"] ) then
				self.BattlepayItemTexture:SetDesaturated(true)
				self.BattlepayItemTexture:SetShown(true)

			elseif(bag.itemSubClassID == Bag.ContainerSubClassID["Leatherworking Bag"]  ) then
				self.BattlepayItemTexture:SetDesaturated(true)
				self.BattlepayItemTexture:SetShown(true)
			end
		elseif ( bag.itemClassID == Bag.ClassID["Quiver"] ) then
			self.BattlepayItemTexture:SetDesaturated(true)
			self.BattlepayItemTexture:SetShown(true)
		end
	end
...
function ItemSlot:GetBagInfo()
	return Addon:GetBagInfo(self:GetOwner(), self:GetBag())
end
