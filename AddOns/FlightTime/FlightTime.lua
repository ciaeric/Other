...
function FlightTime_OnLoad(self) 
...
function FT_ShowTooltip()
    ...
    if (taxiMapIsOpen == 1) then
        ...
        local numTaxiSpots = NumTaxiNodes()
        for taxiNode = 1, numTaxiSpots do
            local name = TaxiNodeName(taxiNode)
            local nodetype = TaxiNodeGetType(taxiNode)
...
function FlightTime_UpdateTime()
    ...
    if (FT_onFlight == 1) then
        if (FTDB[pointString]) then
            ...
            FT_FormatDisplay(1)
            FlightTime_Text:SetText("Recording : " .. FT_startPoint .. " to " .. FT_endPoint);
        end
    end
end
